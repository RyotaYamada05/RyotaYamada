...
//ビームクラスの初期化処理
//=============================================================================
HRESULT CBeam::Init(const D3DXVECTOR3 pos, const D3DXVECTOR3 size)
{
	//デバイスの取得
	LPDIRECT3DDEVICE9 pD3DDevice = CManager::GetRenderer()->GetDevice();
...
void CBeam::Collision(void)
{
	...
	if (pEnemy != NULL)
	{
		...
		//爆発の生成
		CExplosion::Create(enemyPos, EXPLOSION_SIZE*2.5f, COLOR_GREEN, PRIORITY_UI);
	}
	...
	if (pBoss != NULL)
	{
		...
		//爆発の生成
		CExplosion::Create(bossPos, EXPLOSION_SIZE, COLOR_GREEN, PRIORITY_UI);
	}
	...
	if (pBullet != NULL)
	{
		...
		if (pBullet->GetUser() == CBullet::BULLET_USER_ENEMY)
		{
			...
			//爆発の生成
			CExplosion::Create(bulletPos, EXPLOSION_SIZE, COLOR_GREEN, PRIORITY_UI);
		}
	}
}
