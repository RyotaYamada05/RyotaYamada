...
void CPlayer::Update(void)
{
	...
	//スキルポイントが100より大きく、かつMキーまたはジョイスティックXボタンが押されたら
...
void CPlayer::Moving(void)
{
	...
	{
		m_pos.x = SCREEN_WIDTH - (m_size.x / 2);
	}
	if (m_pos.x - (m_size.x / 2) <= 0)
	{
		m_pos.x = 0 + m_size.x / 2;
	}
	if (m_pos.y + (m_size.y / 2) >= SCREEN_HEIGHT)
	{
		m_pos.y = SCREEN_HEIGHT - (m_size.y / 2);
	}
	if (m_pos.y - (m_size.y / 2) <= UNDER_UI_SIZE_Y)
	{
		m_pos.y = UNDER_UI_SIZE_Y + m_size.y / 2;
	}
}
