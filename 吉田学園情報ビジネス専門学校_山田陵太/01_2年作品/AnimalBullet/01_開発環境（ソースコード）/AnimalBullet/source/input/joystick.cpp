...
HRESULT CInputJoyStick::InIt(HINSTANCE hInstance, HWND hWnd)
{
	...
	{
#ifdef _DEBUG
		MessageBox(NULL, "ジョイスティックが見つかりませんでした", "警告", MB_TASKMODAL | MB_ICONEXCLAMATION | MB_OK);
#endif
		Uninit();
		return E_FAIL;
	}
	...
	if (m_pJoyDevice)
	{
		...
		{
			MessageBox(hWnd, "ジョイスティックデバイスのデータフォーマットの設定が失敗しました", "警告", MB_OK);
		...
		{
			MessageBox(hWnd, "ジョイスティックデバイスの協調モードの設定が失敗しました", "警告", MB_OK);
			m_pJoyDevice->Release();
			return DIENUM_STOP;
		}
...
#ifdef _DEBUG
		MessageBox(hWnd, "ジョイスティックデバイス作成に成功", "報告", MB_OK);
#endif 
		//制御開始
		if (!m_pJoyDevice)
		{
...
//=============================================================================
DIJOYSTATE CInputJoyStick::GetJoyStickInfo(void)
{
	DIJOYSTATE js;
	SecureZeroMemory(&js, sizeof(js));
