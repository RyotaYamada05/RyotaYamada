...
#define RANKING_DATA_FILE "data/Text/ranking.txt"	//ランキングファイルのパス
...
void CRanking::Uninit(void)
{
	for (int nCntRanking = 0; nCntRanking < RANKING_NUM; nCntRanking++)
	{
		...
		{
			//ナンバーの終了
			if (m_apNumber[nCntRanking][nCntNum])
			{
				//終了処理
...
void CRanking::SetNumber(void)
{
	...
	{
			//各桁の値を求める
			int nScore = (int)powf(fRadix, (float)SCORE_MAX_NUM - fIndex);
			int nScore2 = (int)powf(fRadix, (float)SCORE_MAX_NUM - fIndex - 1);
