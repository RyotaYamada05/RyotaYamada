...
//マクロ定義
//=============================================================================
#define LOGO_POS D3DXVECTOR3(SCREEN_WIDTH / 2, 200.0f, 0.0f)	//ロゴの位置
#define LOGO_MOVE_SPEED 2.0f	//ロゴの移動速度
#define EFFECT_SIZE_X 100.0f	//エフェクトXサイズ
#define EFFECT_SIZE_Y 100.0f	//エフェクトYサイズ
#define EFFECT_SIZE D3DXVECTOR3(EFFECT_SIZE_X, EFFECT_SIZE_Y, 0.0f)	//エフェクトサイズ
...
{
	//タイトル背景用ポリゴンの生成
	m_apPolygon[TITLE_TYPE_BG] = CPolygon::Create(SCREEN_CNTER_POS, SCREEN_SIZE, CPolygon::TEX_TYPE_TITLE);
...
void CTitle::Update(void)
{
	...
	if (m_nCntEffecter % 120 == 0)
	{
		...
		m_apEffect[0]->SetTexType(CEffect::EFFECT_TYPE_CHEETAH);
		...
		m_apEffect[1]->SetTexType(CEffect::EFFECT_TYPE_GORILLA);
		...
		m_apEffect[2]->SetTexType(CEffect::EFFECT_TYPE_TURTLE);
	}
	m_nCntEffecter++;

	//生成ができたら
	if (m_apPolygon[TITLE_TYPE_ENTER])
	{
		//ENTERキーが押されたら
...
bool CTitle::DecisionState(void)
{
	...
	case TITLE_STATE_LOGO_MOVE:
		//ENTERキーまたはジョイスティックBボタンが押されたとき
		if (CManager::GetKeyborad()->GetKeyBoardTrigger(DIK_RETURN) || 
			CManager::GetJoyStick()->GetJoyStickTrigger(CInputJoyStick::JOY_BUTTON_B))
		{
			//ロゴの位置を強制的に固定位置へ移動
			m_pos = LOGO_POS;
