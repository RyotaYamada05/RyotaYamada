...
void CBullet::Update(void)
{
	...
	{
		//エフェクト生成
		CEffect::Create(m_pos, m_size, m_col, 10, CEffect::EFFECT_TYPE_BULLET);
	}
	//位置に移動量を加算
	m_pos += m_move;
...
bool CBullet::BulletCollision(void)
{
	...
	case BULLET_USER_ENEMY:
		...
		//プレイヤーと当たったか情報を取得
		pPlayer = (CPlayer*)CScene2D::JudgeCollision(OBJTYPE_PLAYER, m_pos, m_size);
	...
	case BULLET_USER_PLAYER:
		...
		//敵と当たったか情報を取得
		pEnemy = (CEnemy*)CScene2D::JudgeCollision(OBJTYPE_ENEMY, m_pos, m_size);
		...
		//ボスと当たったか情報を取得
		pBoss = (CBoss*)CScene2D::JudgeCollision(OBJTYPE_BOSS, m_pos, m_size);
