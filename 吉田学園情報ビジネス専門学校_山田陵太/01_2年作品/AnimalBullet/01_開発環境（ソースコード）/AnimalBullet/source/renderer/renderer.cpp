...
CRenderer::CRenderer()
{
	...
	m_pD3DDevice = NULL;	//Direct3Dオブジェクトのデバイス情報
#ifdef _DEBUG
...
HRESULT CRenderer::Init(HWND hWnd, bool bWindow)
{
	...
	d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;		// 映像信号に同期してフリップする（フロントバッファとZバッファの切り替え方法）
	d3dpp.EnableAutoDepthStencil = TRUE;			// デプスバッファ（Ｚバッファ）とステンシルバッファを作成するかどうか
	d3dpp.AutoDepthStencilFormat = D3DFMT_D16;		// デプスバッファとして16bitを使う
	d3dpp.Windowed = bWindow;						// ウィンドウモード（trueがウィンドウモード・falseがフルスクリーンモード）
	...
	// レンダーステートの設定
	m_pD3DDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);	//裏面を（左回り）をカリング
